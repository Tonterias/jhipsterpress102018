...
public interface CelebRepository extends JpaRepository<Celeb, Long>, JpaSpecificationExecutor<Celeb> {
    ...
    @Query(value = "select distinct celeb from Celeb celeb left join fetch celeb.communities left join fetch celeb.umxms",
    ...
    @Query("select celeb from Celeb celeb left join fetch celeb.communities left join fetch celeb.umxms where celeb.id =:id")
