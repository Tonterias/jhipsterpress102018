...
public class InterestResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllInterestsByUmxmIsEqualToSomething() throws Exception {
        // Initialize the database
        Umxm umxm = UmxmResourceIntTest.createEntity(em);
        em.persist(umxm);
        em.flush();
        interest.addUmxm(umxm);
        interestRepository.saveAndFlush(interest);
        Long umxmId = umxm.getId();

        // Get all the interestList where umxm equals to umxmId
        defaultInterestShouldBeFound("umxmId.equals=" + umxmId);

        // Get all the interestList where umxm equals to umxmId + 1
        defaultInterestShouldNotBeFound("umxmId.equals=" + (umxmId + 1));
    }
